...
public class StreamingIntegerTest extends BaseStreamingTest {
	...
	protected long timeReadLoop(int fields, int fieldsPerGroup, int maxMPapBytes, 
		...
		testCatalog.clientConfig();
		FASTReaderInterpreterDispatch fr = new FASTReaderInterpreterDispatch(testCatalog, RingBuffers.buildRingBuffers(new RingBuffer(new RingBufferConfig((byte)15, (byte)7, testCatalog.ringByteConstants(), testCatalog.getFROM())).initBuffers()));
