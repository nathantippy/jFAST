...
public abstract class FASTWriterDispatchTemplates extends FASTEncoder {
    ...
    public FASTWriterDispatchTemplates(final TemplateCatalogConfig catalog, PipeBundle ringBuffers) {
        super(catalog);
    }    
    ...
    protected void genWritePreamble(int fieldPos, PrimitiveWriter writer, int[] rbB, int rbMask, PaddedLong rbPos, FASTEncoder dispatch) {
        {
            ...
            while (i < s) {
                ...
                int d = Pipe.readInt(rbB, rbMask, rbPos.value +  p);;
    ...
    protected void genWriteTextDeltaOptional(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
        {
    ...
    protected void genWriteTextTailOptional(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
        {
    ...
    public void genWriteBytesDelta(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
        {
            ...
            //count matching front or back chars
            int headCount = LocalHeap.countHeadMatch(target,Pipe.byteBuffer(rbRingBuffer),offset,length,rbRingBuffer.byteMask,byteHeap);
            int tailCount = LocalHeap.countTailMatch(target,Pipe.byteBuffer(rbRingBuffer),offset+length,length,rbRingBuffer.byteMask,byteHeap);
            if (headCount>tailCount) {
                int trimTail = LocalHeap.length(target,byteHeap)-headCount;
    ...
    public void genWriteBytesTail(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
        {
            ...
            int headCount = LocalHeap.countHeadMatch(target,value,offset,length,rbRingBuffer.byteMask,byteHeap);
    ...
    public void genWriteBytesCopyOptional(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
    	{
    ...
    public void genWriteBytesDeltaOptional(int target, int fieldPos, PrimitiveWriter writer, LocalHeap byteHeap, Pipe rbRingBuffer) {
        {
            ...
            if (length<0) {            
                ...
                //count matching front or back chars
                int headCount = LocalHeap.countHeadMatch(target,value,offset,length,rbRingBuffer.byteMask,byteHeap);
                int tailCount = LocalHeap.countTailMatch(target,value,offset+length,length,rbRingBuffer.byteMask,byteHeap);
                if (headCount>tailCount) {
                    int trimTail = LocalHeap.length(target,byteHeap)-headCount;
    ...
    protected void genWriteIntegerSignedDefault(int constDefault, int fieldPos, PrimitiveWriter writer, int[] rbB, int rbMask, PaddedLong rbPos) {
        {
            int value = Pipe.readInt(rbB, rbMask, rbPos.value +  fieldPos);
    ...
    protected void genWriteIntegerSignedNone(int target, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        PrimitiveWriter.writeIntegerSigned(rIntDictionary[target] = Pipe.readInt(rbB, rbMask, rbPos.value +  fieldPos), writer);
    }
    ...
    protected void genWriteIntegerUnsignedNone(int target, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        PrimitiveWriter.writeIntegerUnsigned(rIntDictionary[target] = Pipe.readInt(rbB, rbMask, rbPos.value +  fieldPos), writer);
    }
    ...
    protected void genWriteIntegerSignedConstantOptional(int valueOfNull, int fieldPos, PrimitiveWriter writer, int[] rbB, int rbMask, PaddedLong rbPos) {
        PrimitiveWriter.writePMapBit(valueOfNull==Pipe.readInt(rbB, rbMask, rbPos.value +  fieldPos) ? (byte)0 : (byte)1, writer);  // 1 for const, 0 for absent
    }
    ...
    protected void genWriteIntegerSignedDeltaOptional(int target, int source, int fieldPos, int valueOfNull, PrimitiveWriter writer, int[] rIntDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        
        {
            ...
            if (value == valueOfNull) {
                rIntDictionary[target] = 0; //for none and delta
                PrimitiveWriter.writeNull(writer);
            } else {
                long delta = value - rIntDictionary[source];
    ...
    //None
      ...
      protected void genWriteDecimalDefaultOptionalNone(int exponentSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, int[] rIntDictionary, FASTEncoder dispatch) {
      {
        int exponentValue = Pipe.primaryBuffer(rbRingBuffer)[rbRingBuffer.mask & (int)(Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos)];  
    ...
    protected void genWriteDecimalIncrementOptionalNone(int exponentTarget, int exponentSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
        {
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
            if (exponentValueOfNull == exponentValue) {
                if (0 == rIntDictionary[exponentSource]) { // stored value was null;
                    PrimitiveWriter.writePMapBit((byte)0, writer);
                } else {
    ...
    protected void genWriteDecimalCopyOptionalNone(int exponentTarget, int exponentSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                if (0 == rIntDictionary[exponentSource]) { // stored value was null;
    ...
    protected void genWriteDecimalConstantOptionalNone(int exponentValueOfNull, int mantissaTarget, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
        { 
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull==exponentValue) {
    ...
    protected void genWriteDecimalDeltaOptionalNone(int exponentTarget, int mantissaTarget, int exponentSource, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                rIntDictionary[exponentTarget] = 0;
    ...
    protected void genWriteDecimalNoneOptionalNone(int exponentTarget, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                rIntDictionary[exponentTarget] = 0;
    ...
    // DEFAULTS
     ...
     protected void genWriteDecimalDefaultOptionalDefault(int exponentSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, int[] rIntDictionary, FASTEncoder dispatch) {
      {
        int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
        if (exponentValueOfNull == exponentValue) {
            if (0 == rIntDictionary[exponentSource]) { // stored value was null;
    ...
    protected void genWriteDecimalIncrementOptionalDefault(int exponentTarget, int exponentSource, int mantissaTarget, int exponentValueOfNull, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
        {
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
            if (exponentValueOfNull == exponentValue) {
                if (0 == rIntDictionary[exponentSource]) { // stored value was null;
    ...
    protected void genWriteDecimalCopyOptionalDefault(int exponentTarget, int exponentSource, int mantissaTarget, int exponentValueOfNull, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                if (0 == rIntDictionary[exponentSource]) { // stored value was null;
    ...
    protected void genWriteDecimalConstantOptionalDefault(int exponentValueOfNull, int mantissaTarget, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, FASTEncoder dispatch) {
        { 
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull==exponentValue) {
                PrimitiveWriter.writePMapBit((byte)0, writer);  // 1 for const, 0 for absent
    ...
    protected void genWriteDecimalDeltaOptionalDefault(int exponentTarget, int mantissaTarget, int exponentSource, int exponentValueOfNull, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                rIntDictionary[exponentTarget] = 0;
    ...
    protected void genWriteDecimalNoneOptionalDefault(int exponentTarget, int mantissaTarget, int exponentValueOfNull, long mantissaConstDefault, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
        {   
            int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask, Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
            if (exponentValueOfNull == exponentValue) {
                rIntDictionary[exponentTarget] = 0;
    ...
    protected void genWriteDecimalDefaultOptionalIncrement(int exponentSource, int mantissaSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
     {
       int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
       if (exponentValueOfNull == exponentValue) {
           if (0 == rIntDictionary[exponentSource]) { // stored value was null;
   ...
   protected void genWriteDecimalIncrementOptionalIncrement(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       {
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
           if (exponentValueOfNull == exponentValue) {
               if (0 == rIntDictionary[exponentSource]) { // stored value was null;
   ...
   protected void genWriteDecimalCopyOptionalIncrement(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       {   
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
           if (exponentValueOfNull == exponentValue) {
               if (0 == rIntDictionary[exponentSource]) { // stored value was null;
   ...
   protected void genWriteDecimalConstantOptionalIncrement(int exponentValueOfNull, int mantissaSource, int mantissaTarget, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       { 
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
           if (exponentValueOfNull==exponentValue) {
               PrimitiveWriter.writePMapBit((byte)0, writer);  // 1 for const, 0 for absent
   ...
   protected void genWriteDecimalDeltaOptionalIncrement(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentSource, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       {   
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
           if (exponentValueOfNull == exponentValue) {
               rIntDictionary[exponentTarget] = 0;
   ...
   protected void genWriteDecimalNoneOptionalIncrement(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       {   
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
           if (exponentValueOfNull == exponentValue) {
               rIntDictionary[exponentTarget] = 0;
   ...
   protected void genWriteDecimalDefaultOptionalCopy(int exponentSource, int mantissaSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
       {
         int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
         if (exponentValueOfNull == exponentValue) {
             if (0 == rIntDictionary[exponentSource]) { // stored value was null;
     ...
     protected void genWriteDecimalIncrementOptionalCopy(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
         {
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
             if (exponentValueOfNull == exponentValue) {
                 if (0 == rIntDictionary[exponentSource]) { // stored value was null;
     ...
     protected void genWriteDecimalCopyOptionalCopy(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
         {   
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
             if (exponentValueOfNull == exponentValue) {
                 if (0 == rIntDictionary[exponentSource]) { // stored value was null;
     ...
     protected void genWriteDecimalConstantOptionalCopy(int exponentValueOfNull, int mantissaSource, int mantissaTarget, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
         { 
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
             if (exponentValueOfNull==exponentValue) {
                 PrimitiveWriter.writePMapBit((byte)0, writer);  // 1 for const, 0 for absent
     ...
     protected void genWriteDecimalDeltaOptionalCopy(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentSource, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
         {   
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
             if (exponentValueOfNull == exponentValue) {
                 rIntDictionary[exponentTarget] = 0;
     ...
     protected void genWriteDecimalNoneOptionalCopy(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
         {   
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
             if (exponentValueOfNull == exponentValue) {
                 rIntDictionary[exponentTarget] = 0;
   ...
   //constant
     ...
     protected void genWriteDecimalDefaultOptionalConstant(int exponentSource, int mantissaSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, int[] rIntDictionary, FASTEncoder dispatch) {
         {
           int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
           if (exponentValueOfNull == exponentValue) {
               if (0 == rIntDictionary[exponentSource]) { // stored value was null;
       ...
       protected void genWriteDecimalIncrementOptionalConstant(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
           {
               int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
               if (exponentValueOfNull == exponentValue) {
                   if (0 == rIntDictionary[exponentSource]) { // stored value was null;
       ...
       protected void genWriteDecimalCopyOptionalConstant(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
           {   
               int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
               if (exponentValueOfNull == exponentValue) {
                   if (0 == rIntDictionary[exponentSource]) { // stored value was null;
       ...
       protected void genWriteDecimalConstantOptionalConstant(int exponentValueOfNull, int mantissaSource, int mantissaTarget, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, FASTEncoder dispatch) {
           { 
               int exponentValue = Pipe.primaryBuffer(rbRingBuffer)[rbRingBuffer.mask & (int)(Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos)]; 
               if (exponentValueOfNull==exponentValue) {
                   PrimitiveWriter.writePMapBit((byte)0, writer);  // 1 for const, 0 for absent
       ...
       protected void genWriteDecimalDeltaOptionalConstant(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentSource, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
           {   
               int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
               if (exponentValueOfNull == exponentValue) {
                   rIntDictionary[exponentTarget] = 0;
       ...
       protected void genWriteDecimalNoneOptionalConstant(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, FASTEncoder dispatch) {
           {   
               int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
               if (exponentValueOfNull == exponentValue) {
                   rIntDictionary[exponentTarget] = 0;
  ...
  //delta
       ...
       protected void genWriteDecimalDefaultOptionalDelta(int exponentSource, int mantissaSource, int mantissaTarget, int exponentConstDefault, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, int[] rIntDictionary, FASTEncoder dispatch) {
           {
             int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
             if (exponentValueOfNull == exponentValue) {
                 if (0 == rIntDictionary[exponentSource]) { // stored value was null;
         ...
         protected void genWriteDecimalIncrementOptionalDelta(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
             {
                 int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos);  
                 if (exponentValueOfNull == exponentValue) {
                     if (0 == rIntDictionary[exponentSource]) { // stored value was null;
         ...
         protected void genWriteDecimalCopyOptionalDelta(int exponentTarget, int exponentSource, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, long[] rLongDictionary, int[] rIntDictionary, Pipe rbRingBuffer, long[] longValue, FASTEncoder dispatch) {
             {   
                 int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
                 if (exponentValueOfNull == exponentValue) {
                     if (0 == rIntDictionary[exponentSource]) { // stored value was null;
         ...
         protected void genWriteDecimalConstantOptionalDelta(int exponentValueOfNull, int mantissaSource, int mantissaTarget, int fieldPos, PrimitiveWriter writer, Pipe rbRingBuffer, long[] rLongDictionary, long[] longValue, FASTEncoder dispatch) {
             { 
                 int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
                 if (exponentValueOfNull==exponentValue) {
                     PrimitiveWriter.writePMapBit((byte)0, writer);  // 1 for const, 0 for absent
         ...
         protected void genWriteDecimalDeltaOptionalDelta(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentSource, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
             {   
                 int exponentValue = Pipe.readInt(Pipe.primaryBuffer(rbRingBuffer),rbRingBuffer.mask,Pipe.getWorkingTailPosition(rbRingBuffer)+ fieldPos); 
                 if (exponentValueOfNull == exponentValue) {
                     rIntDictionary[exponentTarget] = 0;
         ...
         protected void genWriteDecimalNoneOptionalDelta(int exponentTarget, int mantissaSource, int mantissaTarget, int exponentValueOfNull, int fieldPos, PrimitiveWriter writer, int[] rIntDictionary, Pipe rbRingBuffer, long[] rLongDictionary, FASTEncoder dispatch) {
             {  
    ...
    protected void genWriteLongUnsignedDefault(long constDefault, int fieldPos, PrimitiveWriter writer, int[] rbB, int rbMask, PaddedLong rbPos) {        
        {
            long value = Pipe.readLong(rbB, rbMask, rbPos.value + fieldPos);
            if (value == constDefault) {
                PrimitiveWriter.writePMapBit((byte)0, writer);
            } else {
                PrimitiveWriter.writePMapBit((byte)1, writer);
    ...
    protected void genWriteLongUnsignedNone(int target, int fieldPos, PrimitiveWriter writer, long[] rLongDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        PrimitiveWriter.writeLongUnsigned(rLongDictionary[target] = Pipe.readLong(rbB, rbMask, rbPos.value + fieldPos), writer);
    }
...
//        }
    protected void genWriteLongUnsignedCopyOptional(long valueOfNull, int target, int source, int fieldPos, PrimitiveWriter writer, long[] rLongDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        {
            ...
            //for copy and inc
            if (value == valueOfNull) {
                if (0 == rLongDictionary[source]) { // stored value was null;
                    PrimitiveWriter.writePMapBit((byte)0, writer);
                } else {
                    rLongDictionary[target] = 0;
                    PrimitiveWriter.writePMapBit((byte)1, writer);
    ...
    protected void genWriteLongUnsignedConstantOptional(long valueOfNull, int target, int fieldPos, PrimitiveWriter writer, int[] rbB, int rbMask, PaddedLong rbPos) {
            PrimitiveWriter.writePMapBit(Pipe.readLong(rbB, rbMask, rbPos.value + fieldPos)==valueOfNull ? (byte)0 : (byte)1, writer);
    }
    ...
    protected void genWriteLongSignedNone(int target, int fieldPos, PrimitiveWriter writer, long[] rLongDictionary, int[] rbB, int rbMask, PaddedLong rbPos) {
        PrimitiveWriter.writeLongSigned(rLongDictionary[target] = Pipe.readLong(rbB, rbMask, rbPos.value + fieldPos), writer);
    }
